...
export interface AuditRow {
  at: number; actor: string; action: string; entity?: string; payload: unknown; ip?: string;
}
...
export const memory: MemoryStore = {
  wallets: new Map(), bots: new Map(), intents: new Map(), txs: new Map(), audit: [],
  prices: {
...
export function upsertWallet(address: string, chain: string): void {
  ...
  if (!memory.wallets.has(key)) memory.wallets.set(key, { address: key, chain, createdAt: Date.now() });
}
export function listBots(wallet: string): BotConfig[] {
  return [...memory.bots.values()].filter((b) => b.wallet === wallet.toLowerCase());
}
export function putBot(bot: BotConfig): BotConfig { memory.bots.set(bot.id, bot); return bot; }
export function createBot(partial: Omit<BotConfig, "id" | "createdAt" | "updatedAt" | "state">): BotConfig {
  const now = Date.now();
  return putBot({ ...partial, wallet: partial.wallet.toLowerCase(), id: newId("bot"), state: "idle", createdAt: now, updatedAt: now });
}
export function putIntent(intent: TradeIntent): TradeIntent { memory.intents.set(intent.id, intent); return intent; }
export function putTx(tx: OnChainTx): OnChainTx { memory.txs.set(tx.hash, tx); return tx; }
export function listTxs(wallet: string): OnChainTx[] {
  return [...memory.txs.values()].filter((t) => t.from.toLowerCase() === wallet.toLowerCase()).sort((a, b) => b.submittedAt - a.submittedAt);
}
export function emergencyPaused(): boolean { return memory.flags.emergency_pause === "true"; }
export function setEmergencyPause(value: boolean): void { memory.flags.emergency_pause = value ? "true" : "false"; }
